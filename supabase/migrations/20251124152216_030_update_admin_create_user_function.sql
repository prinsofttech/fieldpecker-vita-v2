/*
  ...
  1. Changes
    - Add p_reports_to_user_id parameter to admin_create_user function
    - Update function to handle reports_to_user_id field
    - Maintain backward compatibility with existing calls

  2. Purpose
    - Allow setting reporting relationships during user creation
    - Support hierarchical team structure
*/

-- Drop existing function
DROP FUNCTION IF EXISTS admin_create_user(uuid, text, text, uuid, uuid);

-- Recreate with reports_to_user_id parameter
...
BEGIN
  ...
  -- Insert the user
  INSERT INTO users (id, email, full_name, org_id, role_id, status, reports_to_user_id)
  VALUES (p_user_id, p_email, p_full_name, p_org_id, p_role_id, 'active', p_reports_to_user_id);
...
GRANT EXECUTE ON FUNCTION admin_create_user(uuid, text, text, uuid, uuid, uuid) TO authenticated;

COMMENT ON FUNCTION admin_create_user IS 'Allows admins to create users in their organization with reporting relationships. Super admins can create users in any organization.';
